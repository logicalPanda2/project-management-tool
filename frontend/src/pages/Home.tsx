    ...
    return (
        ...
        ? projects.map((project) => (
            <div className="max-w-2xl md:w-4/5 mb-6 p-6 border rounded-lg relative" key={project.title}>
                ...
                <p className="mb-6">This project is <span className="font-semibold">{project.status.toLowerCase()}</span>.</p>
                <Link to={"project"} className="bg-black text-white rounded focus-visible:outline-0 focus-visible:bg-neutral-900 hover:bg-neutral-900 active:bg-neutral-800 px-3 py-1">View</Link>
            </div>
        ))
        : <>
